...
def openTransaction(cmd,comment,order,price,sl,symbol,tp,type,volume):
    ...
    client.commandExecute("tradeTransaction", {
        "cmd": cmd,                               # 0-BUY, 1-SELL, 2-BUY_LIMIT,3-SELL_LIMIT,4-BUY_STOP,5-SELL_STOP
        "customComment": comment,                # some user comment
        "order": order,                             #0 or position number for closing/modifications
        "price": price,     # trade price 
        "sl": sl,           # stop loss
        "symbol": symbol,               # trade symbol
        "tp": tp,           # take profit
        "type": type,                              #0-OPEN,1-PENDING,2-CLOSE,3-MODIFY,4-DELETE
        "volume": volume   # trade volume / lots
        # "cmd": 1,                               # 0-BUY, 1-SELL, 2-BUY_LIMIT,3-SELL_LIMIT,4-BUY_STOP,5-SELL_STOP
        # "customComment": 'none',                # some user comment
        # "order": 0,                             #0 or position number for closing/modifications
        # "price": data['strategy']['price'],     # trade price 
        # "sl": data['strategy']['sl'],           # stop loss
        # "symbol": data['ticker'],               # trade symbol
        # "tp": data['strategy']['tp'],           # take profit
        # "type": 0,                              #0-OPEN,1-PENDING,2-CLOSE,3-MODIFY,4-DELETE
        # "volume": data['strategy']['order_contracts']   # trade volume / lots
        }
    )
    client.disconnect()

def closeTranzaction():
    # get ticker and volume and search for open transactioncs, once we get the transaction ID close the transaction
    # might be acble to use the custom comment field as well
